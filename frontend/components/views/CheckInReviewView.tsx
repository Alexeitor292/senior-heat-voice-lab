...
function InsightSummary({ insight }: { insight: ConversationInsight | null | undefined }) {
  if (!insight) {
    return (
      <div
        className="rounded-lg px-3 py-3"
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <p className="font-semibold mb-1.5" style={{ fontSize: 12.5, color: "#071D3A" }}>
        ...
        <div>
          ...
          <div className="space-y-1.5">
            {insight.follow_up_suggestions.map((item) => (
              <p key={item} style={{ fontSize: 12.5, color: "#667085", lineHeight: 1.45 }}>
                • {item}
              </p>
            ))}
          </div>
      ...
      <div className="px-6 pt-5 pb-5" style={{ background: "white", borderBottom: "1px solid #E8EDF3" }}>
        ...
        <div className="grid grid-cols-4 gap-4">
          <Metric
            ...
            value={
              senior ? (
                <Link href={`/seniors/${senior.id}`} style={{ color: "#1267D8", fontWeight: 600 }}>
                  {senior.name}
                </Link>
              ) : (
          ...
          />
          <Metric label="Created" value={<span className="tabular">{formatDateTime(checkIn.created_at)}</span>} />
          <Metric label="Call Status" value={formatLabel(checkIn.senior_call_status)} />
          <Metric label="Duration" value={<span className="tabular">{formatDuration(checkIn.senior_call_duration_seconds)}</span>} />
        </div>
      </div>
      ...
      <div className="p-6 grid grid-cols-[1fr_360px] gap-5">
        ...
        <div className="space-y-5">
          <Card title="Call Details" icon={<Phone size={15} />}>
            <div className="space-y-3">
              <Metric label="Call SID" value={<span className="tabular break-all">{checkIn.senior_call_sid ?? "—"}</span>} />
              <Metric label="Speech Confidence" value={checkIn.speech_confidence ?? "—"} />
          ...
          <Card title="Safety Flags" icon={<ShieldAlert size={15} />}>
            <div className="space-y-3">
              ...
              <Metric
                ...
                value={checkIn.caregiver_alert_required ? "Required" : "Not required"}
              />
          ...
          {insight && (
            <Card title="Insight Metadata" icon={<Sparkles size={15} />}>
              <div className="space-y-3">
                ...
                <Metric label="Safety Confidence" value={insight.safety_confidence !== null && insight.safety_confidence !== undefined ? `${Math.round(insight.safety_confidence * 100)}%` : "—"} />
                <Metric label="Mood" value={formatLabel(insight.mood_label)} />
                <Metric label="Loneliness" value={formatLabel(insight.loneliness_signal)} />
              </div>
            </Card>
          )}
          ...
          <Card title="Recommended Action" icon={<AlertTriangle size={15} />}>
            <p style={{ fontSize: 12.5, color: "#667085", lineHeight: 1.5 }}>
              {checkIn.recommended_action || "No recommended action was recorded."}
            </p>
          </Card>

          <Card title="Relationship Summary" icon={<HeartPulse size={15} />}>
            <p style={{ fontSize: 12.5, color: "#667085", lineHeight: 1.5 }}>
              {checkIn.caregiver_summary || insight?.relationship_summary || "No relationship summary available."}
            </p>
          </Card>
        </div>
      </div>
