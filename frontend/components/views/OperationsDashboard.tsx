...
interface HeatTrendPoint { date: string; value: number }
...
function HeatTrendChart({ data }: { data: HeatTrendPoint[] }) {
  ...
  const W = 400, H = 110, MAX = 8;
  const pad = { t: 6, b: 0 };
  const chartH = H - pad.t - pad.b;
  const toY = (v: number) => pad.t + chartH - (v / MAX) * chartH;
  const toX = (i: number) => (i / (safeData.length - 1)) * W;

  const points = safeData.map((d, i) => `${toX(i)},${toY(d.value)}`).join(" ");
  const areaPoints = `0,${toY(0)} ${points} ${W},${toY(0)}`;

  const zones = [
    { y: toY(8), h: toY(6) - toY(8), fill: "rgba(229,41,32,0.07)",  label: "Extreme" },
    { y: toY(6), h: toY(4) - toY(6), fill: "rgba(245,158,11,0.08)", label: "High" },
    { y: toY(4), h: toY(2) - toY(4), fill: "rgba(18,103,216,0.06)", label: "Moderate" },
    { y: toY(2), h: toY(0) - toY(2), fill: "rgba(34,199,201,0.07)", label: "Low" },
  ];
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} style={{ display: "block" }}>
        ...
        {safeData.map((d, i) => (
          <g key={i}>
            <circle cx={toX(i)} cy={toY(d.value)} r={4} fill="white" stroke="#22C7C9" strokeWidth={2} />
          </g>
        ))}
      </svg>
      {/* X-axis */}
      <div className="flex justify-between mt-1.5">
        {safeData.map((d) => (
          <span key={d.date} className="tabular" style={{ fontSize: 10, color: "#94A8BC" }}>
            {d.date.replace("May ", "")}
          </span>
        ))}
      </div>
        ...
        {(["Low", "Moderate", "High", "Extreme"] as const).map((lbl) => (
          <div key={lbl} className="flex items-center gap-1.5">
            <span
              ...
              style={{ width: 6, height: 6, display: "inline-block", background: riskColor(lbl) }}
            />
        ...
        {[
          {
            ...
            bg: "#EFF6FF", border: "#BFDBFE", iconBg: "rgba(18,103,216,0.08)",
          },
          {
            icon: <AlertTriangle size={16} style={{ color: "#F59E0B" }} />,
            value: summary.needOutreach,
            label: "Need Outreach",
            bg: "#FFF7ED", border: "#FED7AA", iconBg: "rgba(245,158,11,0.1)",
          },
          {
            icon: <AlertTriangle size={16} style={{ color: "#E52920" }} />,
            value: summary.criticalAlerts,
            label: "Critical Alerts",
            bg: "#FEF2F2", border: "#FECACA", iconBg: "rgba(229,41,32,0.08)",
          },
        ].map((card) => (
          <div
            key={card.label}
          ...
          >
            ...
            <div>
              <div
                ...
                style={{ fontSize: 26, color: "#071D3A", letterSpacing: "-0.04em" }}
              >
                {card.value}
              </div>
          ...
          <div className="rounded-xl overflow-hidden" style={{ background: "white", boxShadow: CARD_SHADOW }}>
            ...
            >
              <div>
                ...
                <p className="mt-0.5" style={{ fontSize: 12, color: "#667085" }}>
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid #F1F5F9", background: "#FAFBFC" }}>
                  {["#", "Senior", "Location", "Risk", "Next Action"].map((col) => (
                    <th key={col} className="text-left px-5 py-2.5 label-caps">{col}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {priorities.map((p, i) => (
                  <tr
                    key={p.seniorId}
                    className="transition-interactive"
                    style={{ borderBottom: i < priorities.length - 1 ? "1px solid #F8FAFC" : undefined }}
                  ...
                  >
                    <td className="px-5 py-3">
                      <span
                        ...
                        style={{
                          ...
                          background: p.risk === "High" || p.risk === "Extreme" ? "#F59E0B" : "#1267D8",
                        }}
                      >
                        {p.rank}
                      </span>
                    ...
                    <td className="px-5 py-3">
                      <ActionButton
                        variant={p.action === "Dispatch wellness check" || p.action.includes("wellness") ? "warning" : "outline"}
                        size="sm"
                      >
                        {p.action}
                      </ActionButton>
        ...
        <div className="w-[272px] shrink-0 space-y-4">
          ...
          <div className="rounded-xl overflow-hidden" style={{ background: "white", boxShadow: CARD_SHADOW }}>
            ...
            >
              ...
              <Link href="/seniors" style={{ fontSize: 11.5, color: "#1267D8", fontWeight: 500 }}>
                View all
              </Link>
            </div>
            <div className="px-4 py-1">
              {schedule.map((item, i) => (
                <div
                  ...
                  style={{ borderBottom: i < schedule.length - 1 ? "1px solid #F8FAFC" : undefined }}
                >
                  ...
                  <div>
                    <div className="flex items-center gap-1.5 mb-0.5">
                      ...
                      <span className="font-medium" style={{ fontSize: 12, color: "#071D3A" }}>
                        {item.type}
                      </span>
                    </div>
                    <p style={{ fontSize: 12, color: "#667085" }}>{item.seniorName}</p>
                    <p style={{ fontSize: 11, color: "#94A8BC" }}>{item.location}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="rounded-xl overflow-hidden" style={{ background: "white", boxShadow: CARD_SHADOW }}>
            ...
            <div className="px-4 py-1">
              {alerts.filter((a) => !a.acknowledged).slice(0, 3).map((alert, i, arr) => (
                <div
                  key={alert.id}
                  className="flex items-start gap-2.5 py-2.5"
                  style={{ borderBottom: i < arr.length - 1 ? "1px solid #F8FAFC" : undefined }}
                >
                  <div
                    className="flex items-center justify-center rounded-md shrink-0 mt-0.5"
                    style={{ width: 22, height: 22, background: riskColor(alert.severity) + "15" }}
                  >
                    <AlertTriangle size={11} style={{ color: riskColor(alert.severity) }} />
                  </div>
                  <div className="min-w-0">
                    <p className="font-medium truncate" style={{ fontSize: 12, color: "#071D3A" }}>
                      {alert.type}
                    </p>
                    <p className="truncate" style={{ fontSize: 11.5, color: "#667085" }}>
                      {alert.seniorName}
                    </p>
                    <p className="tabular" style={{ fontSize: 10.5, color: "#94A8BC" }}>{alert.time}</p>
                  </div>
                </div>
              ))}
            </div>
