...
export async function generateMetadata({ params }: Props) {
  ...
  return { title: senior ? `${senior.name} – Senior Heat Voice Lab` : "Senior Not Found" };
}
...
export default async function SeniorDetailPage({ params }: Props) {
  ...
  return (
    <AppShell>
      <SeniorDetailView senior={senior} timeline={MOCK_TIMELINE} />
    </AppShell>
  );
}
